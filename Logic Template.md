...
**RESEARCH SOURCES [STC: delete this whole block if the project uses no external reference base]:** while *authoring* this document, an external reference base may be consulted for prior art — a curated corpus of notes, a set of reference implementations, a code graph over them, or an agent skill that fronts any of these. Authoring is the *right* phase for that research, because decisions are still open here. Name the base once, below, with where it lives on the machines this project is authored on; nothing else in this document may depend on it.

- **Availability check, once, before you plan around it.** If the base does not resolve, **ask the operator whether they have access to it and where it lives** — do not conclude it is gone from one failed path check (drive letters, mount points and user profiles differ per machine). Its absence is never a blocker. Record the answer here so it is not re-asked every session — but never in `dependency.md` / `requirements.<ext>`: the delivered system must not depend on it.
- **Query, do not bulk-read.** Use whatever index the base offers to find the few files that answer the question, and open only those. A base loaded wholesale is context nobody budgeted for.
...
- **Standing changes at the freeze.** Once this document is frozen the base becomes reference-only and can never override a frozen contract (`Railroad.md` §0, rail 4). Whatever it taught you must be **written into this document as a decision**, because the builder rides this file, not the research behind it. Likewise, never mirror a reference architecture wholesale — an easy-to-query copy of someone else's system makes imitating it *easier*, which is exactly what the interface freeze exists to prevent.
...
The profile decides **which structures exist**. The **Pattern** decides **how much of the standard's apparatus is brought to bear on them**. **This subsection is the authoritative definition** — `supporting/Methodology.md` in the STC repository explains why the mechanism exists, `Railroad.md` §0 lists the build-side apparatus each class runs, and neither restates the rule.
