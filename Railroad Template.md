...
> **Sanctioned reference lookup — only if `Logic.md` §0 names an external reference base (does not widen the reading order above).** The Builder **may** consult that base for *technique and idiom* lookups — how a pattern is normally implemented, what a library's shape is, what a known pitfall looks like, what a working version of this looks like in a real system.
>
> - **Availability, checked once per session, not mid-step.** If the base does not resolve, **ask the user whether they have access to it and where it lives** — do not assume it is gone, and do not treat its absence as a blocker: it is an accelerator, never a prerequisite. A step that *cannot be completed without it* is not a missing repository — it is a `CONTRACT-GAP`.
> - **Query, never bulk-load.** Use the base's index to find the few files that answer the question, and open only those. Cite file paths and symbols, not labels that can shift when the base is rebuilt, and **never maintain or rebuild the base during a build step** — that is not this project's work.
